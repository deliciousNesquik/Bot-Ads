...
def find_overdue_post_json(filename: str, indent: int) -> list:
	...
	for key, value in data.items():
		...
		#запоминаем id поста
		id_post = key
		log_tools.logging(log_file="tool/log.txt", string = f"[json_tools]Обработка поста №{id_post}", time=datetime.now(), limit=10)
		...
		for key, value in value.items():
			...
			if key == "publish_date":
				...
				log_tools.logging(log_file="tool/log.txt", string = f"[json_tools]дата этого поста:{date_post}", time=datetime.now(), limit=10)
				#актуальная дата
				...
				log_tools.logging(log_file="tool/log.txt", string = f"[json_tools]сегодняшняя дата:{date_now}", time=datetime.now(), limit=10)
				#если со времени публикации поста прошло три дня
				if date_now > date_post:
					log_tools.logging(log_file="tool/log.txt", string = f"[json_tools]дата данного поста больше чем сегодняшняя дата", time=datetime.now(), limit=10)
					log_tools.logging(log_file="tool/log.txt", string = f"[json_tools]разница во времени: {abs(date_post - date_now).total_seconds()}\n\n", time=datetime.now(), limit=10)
					...
					log_tools.logging(log_file="tool/log.txt", string = f"[json_tools]данный пост пока что не является просроченным\n\n", time=datetime.now(), limit=10)
